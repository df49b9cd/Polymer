# OmniRelay ↔️ yab Interop Harness

This sample spins up a minimal OmniRelay HTTP server and exercises it with [`yab`](https://github.com/yarpc/yab), Uber's YARPC CLI. It provides a lightweight sanity check that OmniRelay responds to YARPC-style HTTP calls issued by yab.

## Prerequisites

1. Install `.NET` 8/10 SDK (already required for OmniRelay).
...
1. Build the OmniRelay solution.
2. Launch a simple OmniRelay echo service.
...
```bash
# HTTP request only
PORT=9090 bash tests/OmniRelay.YabInterop/run-yab.sh

# gRPC request using the bundled proto
MODE=grpc GRPC_PORT=7070 bash tests/OmniRelay.YabInterop/run-yab.sh

# Exercise both in a single run
MODE=both REQUEST_PAYLOAD='{"message":"multimode"}' bash tests/OmniRelay.YabInterop/run-yab.sh
```

Expected output for the gRPC path resembles:

```
OmniRelay gRPC echo server listening on 127.0.0.1:7070
...
- `tests/OmniRelay.YabInterop/Program.cs` – Minimal OmniRelay HTTP echo service.
- `tests/OmniRelay.YabInterop/run-yab.sh` – Helper script to launch the server and issue a `yab` request.
