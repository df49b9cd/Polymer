...
- Peer metrics emitted by `OmniRelay.Core.Peers` include `omnirelay.peer.inflight`, `omnirelay.peer.successes`, `omnirelay.peer.failures`, and `omnirelay.peer.lease.duration` (histogram). Pair these with the `/omnirelay/introspect` endpoint, which now surfaces per-peer success/failure counts and latency percentiles, to build health dashboards.
