...
namespace YARPCore.Codegen.Protobuf.Core;
...
public sealed class PolymerProtobufGenerator
{
        ...
        Dictionary<string, TypeInfo> typeLookup)
    {
        ...
        public GeneratedFile Emit()
        {
            ...
            builder.AppendLine("using YARPCore.Core;");
            builder.AppendLine("using YARPCore.Core.Clients;");
            builder.AppendLine("using YARPCore.Core.Transport;");
            builder.AppendLine("using Hugo;");
            builder.AppendLine("using YARPCore.Dispatcher;");
            builder.AppendLine("using static Hugo.Go;");
            builder.AppendLine();
            builder.AppendLine($"namespace {_fileContext.Namespace};");
            builder.AppendLine();

            var className = $"{_service.Name}YARPCore";
        ...
        private string BuildOutputFileName()
        {
            ...
            var fileName = $"{baseName}.{_service.Name}.YARPCore.g.cs";
        ...
        {
            builder.AppendLine($"public static void Register{_service.Name}(this global::YARPCore.Dispatcher.Dispatcher dispatcher, I{_service.Name} implementation)");
            builder.AppendLine("{");
            builder.PushIndent();
        ...
        private void GenerateClientClass(IndentedStringBuilder builder, IReadOnlyList<MethodModel> methods)
        {
            ...
            builder.AppendLine("{");
            builder.PushIndent();
            builder.AppendLine("private readonly global::YARPCore.Dispatcher.Dispatcher _dispatcher;");
            ...
            builder.AppendLine();
            builder.AppendLine($"internal {_service.Name}Client(global::YARPCore.Dispatcher.Dispatcher dispatcher, string service, string? outboundKey)");
            builder.AppendLine("{");
            builder.PushIndent();
            builder.AppendLine("ArgumentNullException.ThrowIfNull(dispatcher);");
        ...
        {
            builder.AppendLine($"public static {_service.Name}Client Create{_service.Name}Client(global::YARPCore.Dispatcher.Dispatcher dispatcher, string service, string? outboundKey = null)");
            builder.AppendLine("{");
            builder.PushIndent();
            builder.AppendLine("ArgumentNullException.ThrowIfNull(dispatcher);");
            builder.AppendLine("ArgumentException.ThrowIfNullOrWhiteSpace(service);");
