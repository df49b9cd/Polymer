...
`YARPCore.Codegen.Generator` is the Roslyn incremental generator that powers Polymer's protobuf story. Namespaces remain under `YARPCore.*` for now, but the package/assembly ships as `YARPCore.Codegen.Generator`. It consumes descriptor sets (`.pb` files) and emits:
...
The console project in `src/YARPCore.Codegen.Protobuf` builds `protoc-gen-yarpcore-csharp`. Point `protoc` (or `Grpc.Tools`) at the published binary to generate C# during your build:

```bash
dotnet publish src/YARPCore.Codegen.Protobuf/YARPCore.Codegen.Protobuf.csproj -c Release -o artifacts/codegen
...
Reference the incremental generator as an analyzer and surface descriptor sets via `AdditionalFiles`. The sample below mirrors `tests/YARPCore.Tests/Projects/ProtobufIncrementalSample`:

```xml
<ItemGroup>
  <ProjectReference Include="..\..\src\YARPCore.Codegen.Protobuf.Generator\YARPCore.Codegen.Protobuf.Generator.csproj"
...
See `tests/YARPCore.Tests/Generated/TestService.YARPCore.g.cs` for the full emitted shape.
