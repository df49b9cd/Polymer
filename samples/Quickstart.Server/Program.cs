...
internal static class SampleBootstrap
{
    public static SampleRuntime Build()
    {
        ...
        var httpInbound = new HttpInbound(new[] { "http://127.0.0.1:8080" });
        var grpcInbound = new GrpcInbound(new[] { "http://127.0.0.1:9090" });
    ...
    public static void Register(Dispatcher dispatcher)
    {
        ...
        dispatcher.RegisterJsonUnary<GreetRequest, GreetResponse>(
            ...
            async (context, request) =>
            {
                ...
                return Response<GreetResponse>.Create(response);
            },
            configureProcedure: builder => builder.AddAliases(new[] { "hello::wave" }));
        ...
        dispatcher.RegisterOneway(
            ...
            builder =>
            {
                ...
                builder.Handle(async (request, cancellationToken) =>
                {
                    ...
                    if (decode.IsFailure)
                    {
                        return Err<OnewayAck>(decode.Error!);
                    }

                    var evt = decode.Value;
                    Console.WriteLine($"[telemetry] {evt.Level.ToUpperInvariant()} {evt.Area}: {evt.Message}");

                    return Ok(OnewayAck.Ack(new ResponseMeta(encoding: publishCodec.Encoding)));
                });
            });
